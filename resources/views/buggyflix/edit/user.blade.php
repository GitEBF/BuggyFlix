...
<h1 class="text-xl font-bold text-white capitalize dark:text-white">Modifier {{ $user->email }}</h1>
<form method="post" action="{{ route('UsagersController.update', [$user]) }}">
    @csrf
    @method('PATCH')
    <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
        ...
        <div>
            <label class="text-white dark:text-gray-200" for="role">Mettre admin</label>
            @switch($user->role)
                        @case(1)
                        <input id="role" type="checkbox" name="role" {{ $user->role == 1 ? 'checked' : '' }}>
                            @break
    ...
    <div class="flex justify-end mt-6">
        <button
            class="px-6 py-2 leading-5 text-white transition-colors duration-200 transform bg-red-600 rounded-md hover:bg-red-800 focus:outline-none focus:bg-gray-600">Modifier le Compte</button>
    </div>
</form>
<div>
    </section>
